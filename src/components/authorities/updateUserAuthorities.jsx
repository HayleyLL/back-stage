...
const UpdateUserAuthorities = (props) => {
    ...
    function updateUserAuthorities(userId, codes) {
        userAuthorityApi.updateUserAuthorities(userId, codes)
    ...
    const enterLoading = (index) => {
        ...
        const authCodes = filterNodesByKeys(treeData, checkedKeys.checked).map(node => node.title);
    ...
    function initAuthTree(systemAuthTree) {
        const {list} = systemAuthTree.data;
    ...
    function setUserCheckedKeys(treeData, userAuths) {
        const checkedKeys = findKeysByCodes(treeData, userAuths.map(item => item.code));
        setCheckedKeys(checkedKeys);
    }

    const initUserAuthTree = () => {
        const systemAuthTreeRequest = configApi.getSystemAuthTree();
        const userAuthsRequest = userAuthorityApi.getUserAuthorities(props.match.params.id);
        Promise.all([systemAuthTreeRequest, userAuthsRequest]).then(result => {
